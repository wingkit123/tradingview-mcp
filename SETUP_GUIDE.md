...
TradingView for Windows ships as an MSIX package (Microsoft Store and tvd-packages.tradingview.com install under `C:\Program Files\WindowsApps\`). The MSIX desktop package is not a reliable CDP target: builds may reject `--remote-debugging-port`, strip it, return **"Access is denied"**, or start from a copied folder and then crash. The active `Launch-TradingView-Debug.ps1` detects MSIX and stops with an explicit warning instead of pretending CDP is available.

Use the TradingView web chart through a browser-CDP workflow, or install a legacy standalone `TradingView.exe` if you specifically need desktop CDP.

**Do not register a copied package as a development AppX.** `Add-AppxPackage -Register` against a copied MSIX can fail with `0x80070003` and unregister the working Store installation; recovery requires re-registering the original package from `C:\Program Files\WindowsApps\...` with `-DisableDevelopmentMode`. Do not modify WindowsApps ACLs or use the registration fallback on a working install.
...
| Windows: "Access is denied", `bad option`, or no CDP from an MSIX install | Do not copy/register the package or change `WindowsApps` ACLs; use browser-CDP or a legacy standalone install |
